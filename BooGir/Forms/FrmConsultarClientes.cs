    ...
    public partial class FrmConsultarClientes : Form
    {
        ...
        private async void CargarDataGridAsync()
        {
            ...
            if (result.IsSuccessStatusCode)
            {
                ...
                foreach(Clientes c in lst)
                {
                    dgvConsultar.Rows.Add(c.Dni, $"{ c.Nombre} {c.Apellido}", c.Telefono, c.Direccion); 
                }
            }
        }

        private async void CargarDataFiltradoAsync()
        {
            string url = $"https://localhost:44373/ClientesPorFiltro?dni={Convert.ToInt32(txtDni.Text)}";
            //creamos el httpclient que atravez de este objeto es que se consume la api
            HttpClient cliente = new HttpClient();
            var result = await cliente.GetAsync(url);
            var content = await result.Content.ReadAsStringAsync();
